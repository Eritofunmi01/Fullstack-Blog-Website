...
function WriteBlog() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const general = fetchedCategories.find(
  ...
  const handleSubmit = async (type) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://blug-be-api.onrender.com/api/create-blog",
        {
          method: "POST",
  ...
  const latestBlogs = blogs
    .filter((b) => b.latest && b.status === "published")
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const trending = blogs
    .filter((b) => b.trending && b.status === "published")
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  return (
    <div className="font-serif bg-gray-100 min-h-screen pt-10 flex flex-col lg:flex-row relative">
      ...
      <div className="shadow-3xl w-[90%] lg:w-[60%] mx-auto lg:mx-0 bg-white pb-10 rounded-xl overflow-hidden mb-10 lg:mb-0">
        ...
        <div
          {...getRootProps()}
          className="cursor-pointer border border-dashed border-gray-400 px-4 py-4 mx-5 my-4 text-center rounded-md bg-gray-50 hover:bg-gray-100 transition text-sm text-gray-500"
        >
          <input {...getInputProps()} />
          {isDragActive ? (
        ...
        <input
          ...
          className="border-0 outline-0 ml-5 text-gray-600 placeholder:text-4xl text-3xl p-2 w-[90%]"
          placeholder="Title"
        />
        ...
        <div className="flex flex-wrap gap-4 ml-5 mt-6 items-center">
          <button
            ...
            className="relative bg-gray-600 w-[30%] md:w-[150px] p-2 rounded-full text-white font-semibold text-lg hover:bg-gray-700 transition"
          >
            Show Drafts
            {draftCount > 0 && (
          ...
          <button
            ...
            disabled={loading}
            className="bg-yellow-400 w-[30%] md:w-[150px] p-2 rounded-full text-white font-semibold text-lg hover:bg-yellow-500 transition disabled:opacity-50"
          >
            Save Draft
          </button>
          ...
          <button
            ...
            disabled={loading}
            className="bg-green-600 w-[30%] md:w-[150px] p-2 rounded-full text-white font-semibold text-lg hover:bg-green-700 transition disabled:opacity-50"
          >
            Publish
          </button>
        </div>
      ...
      <div className="hidden lg:block lg:w-[25%] bg-white shadow-lg ml-5 mr-8 rounded-xl overflow-y-scroll scrollbar-none h-[80vh] p-4 [&::-webkit-scrollbar]:hidden">
        {/* Fetch from context */}
        {(() => {
          const { trending, latest } = useContext(BlogContext);
          const navigate = useNavigate();

          const safeTrending = Array.isArray(trending)
            ? trending.slice(0, 2)
            : [];
          const safeLatest = Array.isArray(latest) ? latest.slice(0, 2) : [];
                ...
                <div className="space-y-4">
                  {safeLatest.length ? (
                    safeLatest.map((blog) => (
                      <div
                        key={blog.id}
                        className="border min-h-[5rem] rounded-lg p-3 hover:shadow-md transition cursor-pointer"
                        onClick={() => navigate(`/blog/${blog.id}`)}
                      >
                        {blog.img && (
                          <img
                    ...
                    <p className="text-gray-500">No latest blogs</p>
                  )}
                </div>
              </div>
            </>
